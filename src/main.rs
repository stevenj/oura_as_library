...
mod stdout;
use crate::stdout::Config as StdoutConfig;
use crate::stdout::StdoutUtils;
...
fn bootstrap_sink(sink: DumpSink, input: StageReceiver, utils: Arc<Utils>) -> BootstrapResult {
    match sink {
        DumpSink::Stdout(c) => StdoutUtils::new(c, utils).bootstrap(input),
    }
}
...
fn main() {
    let args = Command::new("app")
        ...
        .subcommand(command_definition())
        .arg_required_else_help(true)
        .get_matches();

    let result = match args.subcommand() {
        Some(("dump", args)) => run(args),
